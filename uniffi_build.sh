HEADERPATH="bindings/dumbpipeFFI.h"
TARGETDIR="target"
...
cargo run --bin uniffi-bindgen generate --library target/aarch64-apple-ios-sim/debug/libdumbpipe.a --language swift --out-dir bindings

echo "Bundling framework"
mkdir -p "${NEW_HEADER_DIR}"
cp "${HEADERPATH}" "${NEW_HEADER_DIR}/"
cp "bindings/dumbpipeFFI.modulemap" "${NEW_HEADER_DIR}/module.modulemap"
